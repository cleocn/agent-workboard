...
RELEASE_0_1_IDENTITY = {
    "packageVersion": "0.1.0",
    "sourceCommit": "133365c0cd7583f2cadf21f10dcc82d9fc7bba7f",
    "sourceTree": "e142182c64845fb64e09814e5fcb85b693a21d2c",
    "sourceTag": "v0.1.0",
}
RELEASE_0_2_IDENTITY = {
    "packageVersion": "0.2.0",
    "sourceCommit": "e813397bce0f75ba70176608a7ca9a50fa2f26ea",
    "sourceTree": "d47fea22c9f9000bdb4d44e6682336ab348caf06",
    "sourceTag": "v0.2.0",
}
SUPPORTED_UPGRADE_SOURCES = (RELEASE_0_1_IDENTITY, RELEASE_0_2_IDENTITY)
    ...
    if parsed.scheme == "file":
        ...
        if actual != digest:
          ...
          not any(parsed.path.startswith("/cleocn/agent-workboard/releases/download/{0}/".format(tag))
                  for tag in ("v0.1.0", "v0.2.0", "v0.2.1"))):
...
def _forward_refusal_step(reason, root, wheel, with_codex):
    ...
    elif "outside the explicit" in reason or "does not match the running" in reason:
...
def _upgrade_preflight(path, wheel_path, with_codex, operation):
    ...
    try:
        ...
        if (target_identity != BUILD_IDENTITY or
                BUILD_IDENTITY.get("packageVersion") != "0.2.1" or
                BUILD_IDENTITY.get("sourceTag") != "v0.2.1"):
            raise LiteError("upgrade target wheel does not match the running 0.2.1 release")
        ...
        if current_identity not in SUPPORTED_UPGRADE_SOURCES:
            raise LiteError("upgrade source identity is outside the explicit 0.1.0/0.2.0 to 0.2.1 matrix")
        ...
        changed = []
        created = []
        untouched = [{"path": os.path.relpath(os.path.join(_awb(root), "project.md"), root).replace(os.sep, "/")},
                     {"path": os.path.relpath(database, root).replace(os.sep, "/")}] + codex_untouched
        effective = []
        ...
        projected = changed + created
        paths = {"backup": sorted(backup, key=lambda item: item["path"]),
                 "replaced": sorted(changed, key=lambda item: item["path"]),
...
def _write_upgrade(plan):
    ...
    try:
        ...
        manifest = {
            ...
            "actions": actions,
            "retained": [{"action": "RETAIN", "path": os.path.relpath(plan["database"], root).replace(os.sep, "/"),
...
def _expected_rollback_material(root, database, manifest, backup_root):
    """Reconstruct the only mutations the bounded upgrade to 0.2.1 can make."""
    ...
    expected_retained = [{
        "action": "RETAIN",
        "path": os.path.relpath(database, root).replace(os.sep, "/"),
        "backup": os.path.relpath(database, root).replace(os.sep, "/"),
    }]
    ...
    _manifest_target(backup_root, expected_retained[0]["backup"], "retained database backup")
...
def _load_rollback(path, manifest_path, operation):
    ...
    try:
        ...
        if os.path.lexists(staging):
            ...
            return {"result": result, "root": root, "manifest": manifest,
...
def _apply_rollback_action(action):
...
def _restore_post_upgrade(action, staged):
...
def _recover_rollback(plan):
    try:
        ...
        for action, saved in zip(plan["actions"], staged):
            _restore_post_upgrade(action, saved)
...
def upgrade_project(path, wheel_path=None, with_codex=False, check=False, rollback_manifest=None):
    """Check, execute, or exactly roll back a bounded upgrade to 0.2.1."""
