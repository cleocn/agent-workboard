...
class ReleaseBuildTest(unittest.TestCase):
    def _wheel_from_build(self, repository, destination):
        ...
        prefix = "agent_workboard-0.2.1.dist-info"
    ...
    def _assert_runbook_semantics(self, raw):
        ...
        for identity in ("`0.1.0/v0.1.0`", "`0.2.0/v0.2.0`", "`0.2.1/v0.2.1`"):
    ...
    def test_tagged_build_and_no_git_sdist_preserve_identity_and_templates(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual("0.2.1", direct_identity["packageVersion"])
            self.assertEqual("v0.2.1", direct_identity["sourceTag"])
            ...
            for sequence in ("one", "two"):
                ...
                subprocess.check_call(["git", "checkout", "--quiet", "v0.2.1"], cwd=checkout)
                subprocess.check_call([sys.executable, "setup.py", "sdist"], cwd=checkout, env=environment,
                                      stdout=subprocess.DEVNULL)
                archive = os.path.join(checkout, "dist", "agent-workboard-0.2.1.tar.gz")
            ...
            source_tree = os.path.join(extracted, "agent-workboard-0.2.1")
            ...
            with open(os.path.join(source_tree, "build", "lib", "agent_workboard", "resources", "codex",
                ...
                self.assertEqual(root_runbook, handle.read())
            no_git_wheel = os.path.join(source_tree, "dist", "agent_workboard-0.2.1-py3-none-any.whl")
