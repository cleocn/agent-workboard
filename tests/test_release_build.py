...
class ReleaseBuildTest(unittest.TestCase):
    def _wheel_from_build(self, repository, destination):
        ...
        prefix = "agent_workboard-0.3.1b5.dist-info"
    ...
    def _release_repository(self, root):
        ...
        subprocess.check_call(["git", "tag", "-a", "v0.3.1b5", "-m", "preview"], cwd=repository)
        return repository
    ...
    def test_public_and_spec_manifests_declare_required_paths_without_byte_hashes(self):
        ...
        self.assertIn("ARTIFACT_SECRET_SCAN", manifest["releasePolicy"]["requiredGates"])
        self.assertIn("REACHABLE_OBJECT_CLOSURE", manifest["releasePolicy"]["notRequired"])
    ...
    def test_one_tagged_build_preserves_identity_and_templates(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual("0.3.1b5", direct_identity["packageVersion"])
            self.assertEqual("v0.3.1b5", direct_identity["sourceTag"])
