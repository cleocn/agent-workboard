...
`reviewer`/`gpt-6-astra`/`high`, Convergence=`reviewer`/`gpt-6-astra`/`max`,
and Fast Worker=`worker`/`gpt-5.6-luna`/`medium`. Root Orchestrator is a host
launch policy of `gpt-6-astra`/`high`, not a child spawn.
...
For round 4, select built-in `reviewer` with explicit `gpt-6-astra`/`max` and
...
`gpt-6-astra`/`high`. The upward-only fallbacks are `planner` Terra/high to
`planner` Astra/high, `implementer` Terra/high to `implementer` Astra/high, and
`worker` Luna/medium to `worker` Terra/high. Any unavailable Astra or Convergence
...
For exact b3/b4/b5/b6/b7/b8/b9 upgrades, consume the single recovery-aware b10 nextStep unchanged;
never ask the user to edit SQLite or perform a separate stale cleanup first.
The b10 target owns the frozen `AWB-MIGRATION-GRAPH-v1`; never infer a route from
